...
```bash
pnpm --filter @growth-tracker/client test
```
...
## 1. Email and Password — Basic Controlled Inputs **[Required]**
...
## 3. Role Selector — Radio Buttons as a Styled Toggle **[Required]**
...
## 4. Conditional Show/Hide — Team Name **[Required when role = MANAGER]**
...
## 6. Experience Level — Radio Group **[Required]**
...
## 7. Bio — Textarea with Character Counter **[Optional]**

**Field:** `bio` (max 250 characters)

A free-text field with a live character counter below it. Because the field is optional, the form must still submit correctly when it is left empty.
...
|---|---|
| `bio-input` | `<textarea>` |
| `bio-char-count` | The live counter element — shows `0 / 250`, `5 / 250`, etc. |

**What to explore:**
- Using `maxLength` on a textarea to prevent input beyond the limit at the browser level
- Keeping a counter in sync with input value via derived state rather than a separate state variable
- How to make an optional field truly optional — ensure `bio` is omitted (or sent as `undefined`) when empty, not sent as `""`

---

## 8. Birthdate — Three Selects Combining into One String **[Required]**

**Field:** `birthdate` — three separate `<select>` inputs (year, month, day) that combine into a single `YYYY-MM-DD` string before submission.
...
**Year range:** 1940 to (current year − 10). For example, in 2026 that is 1940–2016.

**Combining the values:**
Before submitting, assemble the string: `YYYY-MM-DD`. Zero-pad month and day to two digits (e.g. June → `"06"`, the 5th → `"05"`).

**What to explore:**
- Deriving a single submission value from multiple independent state variables
- Zero-padding with `String.padStart(2, '0')` vs building a lookup array for month names
- Validating that all three selects have a value before allowing the form to be submitted

---

## 9. Address Groups — Dynamic List **[Optional]**

**Fields:** `addresses[]` — each entry has `label`, `street1`, `street2` (optional), `city`, `zipCode`
...
| `address-zip-input` | The ZIP `<input type="number">` **inside** each address group |
...
- Sending `zipCode` as a **number** (not a string) — use `type="number"` on the input or cast with `Number()` before submission

---

## 10. Collapsible Address Groups — Expand / Collapse
...
2. Sanitise the payload — strip client-only fields (`id`, `isOpen`) from addresses; omit `teamName` entirely when role is `LEARNER`; send `zipCode` as a number; combine birthdate selects into `YYYY-MM-DD`
